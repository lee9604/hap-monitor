package com.kuyuntech.hapmonitor.coreapi.service.core;

import com.kuyuntech.hapmonitor.coreapi.bean.core.DmsGroupBean;
import com.kuyuntech.hapmonitor.coreapi.bean.core.UmsUserBean;
import com.wbspool.fastboot.core.common.bean.PagerBean;
import java.util.List;

/**
 * DmsGroupService
 *
 */
public interface DmsGroupService  {

    

    /**
     * 新增记录
     * @param dmsGroupBean 新增参数
     * @param umsUserBean
     * @return 如果成功返回对应bean，否则返回null
     */
     DmsGroupBean add(DmsGroupBean dmsGroupBean, UmsUserBean umsUserBean);

    /**
     * 更新记录
     * @param dmsGroupBean 更新参数
     * @return 如果成功返回对应bean，否则返回null
     */
     DmsGroupBean update(DmsGroupBean dmsGroupBean);

    /**
     * 删除记录
     * @param dmsGroupBean 删除参数
     * @return 如果成功返回对应bean，否则返回null
     */
    DmsGroupBean delete(DmsGroupBean dmsGroupBean);

    /**
     * 查找一条记录
     * @param dmsGroupBean 查找参数
     * @return 如果成功返回对应bean，否则返回null
     */
    DmsGroupBean find(DmsGroupBean dmsGroupBean);

    /**
     * 通过唯一标识查找记录
     * @param code 用户标识
     * @return 记录存在返回对应的bean，否则返回null
     */
     DmsGroupBean find(String code);

    /**
     * 分页查找多条记录
     * @param dmsGroupBean 查找参数
     * @param pagerBean  分页参数
     * @return 记录存在返回对应的bean集合，否则返回空集合
     */
     List<DmsGroupBean> findAll(DmsGroupBean dmsGroupBean, PagerBean pagerBean);



    /**
     * 查找所有记录
     * @param dmsGroupBean 查找参数
     * @return 记录存在返回对应的bean集合，否则返回空集合
     */
     List<DmsGroupBean> findAll(DmsGroupBean dmsGroupBean);


    /**
     * 统计所有记录
     * @param dmsGroupBean 统计参数
     * @return 记录存在返回对应的bean集合，否则返回空集合
     */
     Long countAll(DmsGroupBean dmsGroupBean);

    /**
     * 分页查找多条记录
     * @param dmsGroupBean 查询条件参数
     * @param pagerBean 分页参数
     * @return 返回查询结果的Pager封装
     */
     PagerBean<DmsGroupBean> findPager(DmsGroupBean dmsGroupBean, PagerBean pagerBean);

    /**
     * 批量删除
     * @param dmsGroupBeans 批量删除元素集合
     */
     void batchDelete(List<DmsGroupBean> dmsGroupBeans);

    /**
     * 批量删除
     * @param codes 唯一标识集合
     */
    void batchDeleteByCodes(List<String> codes);


    PagerBean<DmsGroupBean> findPager(DmsGroupBean dmsGroupBean, PagerBean pagerBean, UmsUserBean umsUserBean);

    List<DmsGroupBean> findAll(DmsGroupBean dmsGroupBean, PagerBean pagerBean, UmsUserBean umsUserBean);
}
